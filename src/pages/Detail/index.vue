  ...
  export default {
    ...
    //  表单元素修改产品个数
      changeSkuNum(event){
        ...
        // 如果用户输入数量非法  出现NaN或数量小于1
        if (isNaN(value)||value<1){
          this.skuNum = 1;
        }else {
          // 对value值取整
          this.skuNum = Math.trunc(value);
        }
      },
      async addShopcar(){
        ...
        try {
          ...
          await this.$router.push({name: "addcartsuccess", query: {skuNum:this.skuNum.toString()}});
        }catch (error){
          //失败
