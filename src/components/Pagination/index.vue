<template>
    <div class="pagination">
        ...
        <button @click="$emit('currentPage',1)" v-if="startAndEnd.start > 1">1</button>
        <button v-if="startAndEnd.start > 2">...</button>

        <!-- 中间连续页码的地方:v-for、数组、对象、数字、字符串 -->
        <button v-for="page in startAndEnd.end" v-if="page>=startAndEnd.start" @click="$emit('currentPage',page)" :class="{active: pageNo===page}">{{page}}</button>
...
export default {
    ...
    computed: {
        ...
        startAndEnd(){
            let start = 0, end = 0; 
            // 解构 totalPage, pagerCount, pageNo
